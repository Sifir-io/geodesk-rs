...
std::unique_ptr<FeatureResult> FeatureStore::query_amenities(
    const std::string& amenity_type,
...
std::unique_ptr<FeatureResult> FeatureStore::query(
    const std::string& goql_query,
    const BoundingBox& bbox) const
{
...
size_t FeatureResult::count() const {
...
std::vector<FeatureData> FeatureResult::to_vector() const {
    return features;
}
...
std::unique_ptr<FeatureResult> query_amenities_in_bbox(
    const FeatureStore& store,
    rust::Str amenity_type,
...
std::unique_ptr<FeatureResult> query_with_goql(
    const FeatureStore& store,
    rust::Str goql_query,
    double west, double south, double east, double north)
{
...
std::unique_ptr<std::vector<FeatureData>> result_to_vec(const FeatureResult& result) {
